...
class Administrator extends CI_Controller {
	function index()
	{
		if (isset($_POST['submit'])) {
			...
			$passlain = $this->input->post('b');
			$password = hash("sha512", md5($this->input->post('b')));
			$password_lain = $this->input->post('b');
			$cek = $this->model_app->cek_login($username, $password,'tbl_users');
			$row = $cek->row_array();
			$total = $cek->num_rows();
			if ($total > 0) {
				$array = array(
	...
	function tesx(){
		$sql = "SELECT * FROM tbl_gis";
		$list = $this->db->query($sql);
		$marker = array();
		foreach ($list->result() as $rw) {
			$marker[] = array(
				'latlng' => $rw->lat.','.$rw->long, // Marker Location
				'popupContent' => $rw->text, // Popup Content
			);
		}
	...
	function tambah_statis()
	{
		...
		if (isset($_POST['submit'])) {
			$data = array(
						...
						'seo_statis'=>seo_title($this->input->post('b')),
	...
	function tambah_menuwebsite(){
		cek_session_admin();
		if (isset($_POST['submit'])){
			$typs = array('id_menu_type'=>$this->db->escape_str($this->input->post('tp')));
			$cekTyps = $this->model_app->view_where('tbl_menu_type',$typs);
			if ($cekTyps->num_rows()<=0) {
				$data = array(
				...
				$cekTypes = $cekTyps->row_array();
				if ($cekTypes['type_menu']=='berita') {
					$typesny = 'berita';
					$data = array(
								'id_menu_type'=>$this->db->escape_str($this->input->post('tp')),
								'id_parent'=>$this->db->escape_str($this->input->post('b')),
				...
				}elseif ($cekTypes['type_menu']=='statis') {
					$typesny = 'statis';
					$data = array(
								'id_menu_type'=>$this->db->escape_str($this->input->post('tp')),
								'id_parent'=>$this->db->escape_str($this->input->post('b')),
				...
				}elseif ($cekTypes['type_menu']=='files') {
					$typesny = 'files';
					$data = array(
								'id_menu_type'=>$this->db->escape_str($this->input->post('tp')),
								'id_parent'=>$this->db->escape_str($this->input->post('b')),
				...
				}elseif ($cekTypes['type_menu']=='gallery') {
					$typesny = 'gallery';
					$data = array(
								'id_menu_type'=>$this->db->escape_str($this->input->post('tp')),
								'id_parent'=>$this->db->escape_str($this->input->post('b')),
	...
	function alutmatra($matra)
	{
		cek_session_admin();
		$get_matra  = str_replace('-', ' ', strtoupper($matra));
		$datas   = array('matra'=>$get_matra);
		$data['record'] = $this->model_app->view_where_ordering('tbl_alutmatra', $datas, 'id', 'DESC');
...
// end alut matra
	...
	function gis()
	{
		$this->template->load('administrator/template','administrator/mod_gis/gis');
	}
	function tambah_gis()
	{
		cek_session_admin();
		if (isset($_POST['submit'])) {
			...
			$data['satker'] = $this->model_app->get_combo("tbl_uosatker", "kode_satker <> ''", "kode_satker", "nama_satker");
			$data['provinsi'] = $this->model_app->get_combo("tbl_wilayah", "kd_prov <> '' AND kd_kab IS NULL", "kd_prov", "provinsi");
	...
	{
		cek_session_admin();
		$data = $this->model_app->get_combo("tbl_uosatker", "kode_satker <> ''", "kode_satker", "nama_satker");
	...
	function getkab()
	{
		cek_session_admin();
		$id =  $_REQUEST['prov'];
		$data = $this->model_app->get_combo("tbl_wilayah", "kd_prov = $id AND kd_kab IS NOT NULL", "kd_kab", "kabupaten");
		echo json_encode($data);
	}
	function getkec()
	{
		...
		$prov =  $_REQUEST['prov'];
		$kab =  $_REQUEST['kab'];
		$data = $this->model_app->get_combo("tbl_wilayah", "kd_prov = $prov AND kd_kab = $kab AND kd_kec IS NOT NULL", "kd_kec", "kecamatan");
		echo json_encode($data);
	}
	function ubah_gis()
	{
		...
		if (isset($_POST['submit'])) {
			...
			$data = array(
							...
							'satker' => $this->model_app->get_combo("tbl_uosatker", "kode_satker <> ''", "kode_satker", "nama_satker"),
							'provinsi' => $this->model_app->get_combo("tbl_wilayah", "kd_prov <> '' AND kd_kab IS NULL", "kd_prov", "provinsi"),
							'kabupaten' => $this->model_app->get_kabedit($id),
							'kecamatan' => $this->model_app->get_kecedit($id)
						);
	...
	// controller tanah (update 2020)
	function tanah($uo, $kat)
	{
		cek_session_admin();
		$get_uo  = str_replace('-', ' ', strtoupper($uo));
		$get_kat = strtoupper($kat);
		$datas   = array('kategori'=>$get_kat, 'uo'=>$get_uo);
		$data['record'] = $this->model_app->view_where_ordering('tbl_tanah', $datas, 'autono', 'DESC');
		$this->template->load('administrator/template','administrator/mod_tanah/tanah',$data);
	}
	function tambah_tanah($uo, $kat)
	{
		cek_session_admin();
		$get_uo  = str_replace('-', ' ', strtoupper($uo));
		$get_kat = strtoupper($kat);
		if (isset($_POST['submit'])) {
			$data = array(
							'kategori'=> $this->db->escape_str($get_kat),
							'uo'=> $this->db->escape_str($get_uo),
			...
			if ($q) {
				redirect('administrator/tanah/'.$uo.'/'.$kat.'/berhasil','refresh');
			}else{
				redirect('administrator/tanah/'.$uo.'/'.$kat.'/gagal','refresh');
			}
		}else{
	...
	{
		cek_session_admin();
		$data['record'] = $this->model_app->view_ordering('tbl_covid19','idcountry','ASC');
		$this->template->load('administrator/template','administrator/mod_covid/covid',$data);
	}
	function tambah_data_covid()
	{
		cek_session_admin();
		if (isset($_POST['submit'])) {
			$data = array(
						...
						'subbenua'=>$this->db->escape_str($this->input->post('b')),
						'negara'=>$this->db->escape_str($this->input->post('c')),
						'code'=>$this->db->escape_str($this->input->post('d')),
						'coord'=>$this->db->escape_str($this->input->post('e')),
						'long'=>$this->db->escape_str($this->input->post('f')),
						'lat'=>$this->db->escape_str($this->input->post('g')),
						'flag'=>1
					);	
			$q = $this->model_app->insert('tbl_covid19', $data);
			if ($q) {
				redirect('administrator/covid/berhasil','refresh');
			}else{
				redirect('administrator/covid/gagal','refresh');
			}
		}else{
			$data['title'] = 'Tambah Data Covid';
			$this->template->load('administrator/template', 'administrator/mod_covid/tambah_covid',$data);
		}
	}
	function uabh_data_covid()
	{
		cek_session_admin();
		$id = $this->uri->segment(3);
		if (isset($_POST['submit'])) {
			...
			$where = array('idcountry'=>$this->input->post('id'));	
			$q = $this->model_app->update('tbl_covid19', $data, $where);
			if ($q) {
				redirect('administrator/covid/berhasil','refresh');
			}else{
				redirect('administrator/covid/gagal','refresh');
			...
			$data= array(
							'title' => 'Ubah Data Covid',
							'row'   => $this->model_app->edit('tbl_covid19', array('idcountry'=>$id))->row_array()
						);
			$this->template->load('administrator/template', 'administrator/mod_covid/ubah_covid',$data);
		}
	}
	function wilayah()
	{
		$this->template->load('administrator/template','administrator/mod_wilayah/wilayah');
	}
	function tambah_wilayah()
	{
		...
		if (isset($_POST['submit'])) {
			$data = array(
				...
				'kd_prov'=>$this->input->post('b'),
				'kd_kab'=>$this->input->post('c'),
				'kd_kec'=>$this->input->post('d'),
				'negara'=>$this->input->post('e'),
				'provinsi'=>$this->input->post('f'),
				'kabupaten'=>$this->input->post('g'),
				'kecamatan'=>$this->input->post('h'),
				'desa'=>$this->input->post('i'),
				'kor_long'=>$this->input->post('j'),
				'kor_lat'=>$this->input->post('k'),
				'keterangan'=>$this->input->post('l'),
				'created_by' => $user,
			);
			$q = $this->model_app->insert('tbl_wilayah',$data);
			if ($q) {
				redirect('administrator/wilayah/berhasil','refresh');
			}else{
				redirect('administrator/wilayah/gagal','refresh');
			}
		}else{
			$data['title'] = 'Tambah Data Master Wilayah';
			$this->template->load('administrator/template','administrator/mod_wilayah/tambah_wilayah',$data);
		}
	}
	function ubah_wilayah()
	{
		cek_session_admin();
		...
		if (isset($_POST['submit'])) {
			...
			$where = array('autono'=>$this->input->post('id'));
			$q = $this->model_app->update('tbl_wilayah',$data,$where);
			if ($q) {
				redirect('administrator/wilayah/berhasil','refresh');
			}else{
				redirect('administrator/wilayah/gagal','refresh');
			...
			$data['title'] = 'Ubah Data Master Wilayah';
			$data['row'] = $this->model_app->edit('tbl_wilayah',array('autono'=>$id))->row_array();
			$this->template->load('administrator/template','administrator/mod_wilayah/ubah_wilayah',$data);
		}
	}
	function hapus_wilayah()
	{
		cek_session_admin();
		$id = array('autono'=>$this->uri->segment(3));
		$q = $this->model_app->delete('tbl_wilayah',$id);
		if ($q) {
				redirect('administrator/wilayah/berhasil','refresh');
		}else{
				redirect('administrator/wilayah/gagal','refresh');
	...
	{
		cek_session_admin();
		$this->template->load('administrator/template','administrator/mod_rs/rs');
	}
	function tambah_rs()
	{
		cek_session_admin();
		if (isset($_POST['submit'])) {
			$data = array(
						  'kd_prov'=>$this->db->escape_str($this->input->post('provinsi')),
						  'kd_kab'=>$this->db->escape_str($this->input->post('kabupaten')),
						  'kd_kec'=>$this->db->escape_str($this->input->post('kecamatan')),
						  'nama_prop' =>$this->db->escape_str($this->input->post('a')),
						  'kode_rs' =>$this->db->escape_str($this->input->post('b')),
						  'nama_rs' =>cetak($this->input->post('c')),
						  'alamat' =>$this->db->escape_str($this->input->post('d')),
						  'telp' =>$this->db->escape_str($this->input->post('e')),
						  'kelas' =>$this->db->escape_str($this->input->post('f')),
						  'jenis_rs' =>$this->db->escape_str($this->input->post('g')),
						  'pemilik' =>$this->db->escape_str($this->input->post('h')),
						  'vvip' =>$this->db->escape_str($this->input->post('i')),
						  'vip' =>$this->db->escape_str($this->input->post('j')),
						  'kls1' =>$this->db->escape_str($this->input->post('k')),
						  'kls2' =>$this->db->escape_str($this->input->post('l')),
						  'kls3' =>$this->db->escape_str($this->input->post('m')),
						  'tahun' =>$this->db->escape_str($this->input->post('n')),
						  'provinsi' =>$this->db->escape_str($this->input->post('prov')),
						  'covid19' =>$this->db->escape_str($this->input->post('o')),
						  'jml_dokter' =>$this->db->escape_str($this->input->post('p')),
						  'jml_perawat' =>$this->db->escape_str($this->input->post('q')),
						  'kor_long' =>$this->db->escape_str($this->input->post('r')),
						  'kor_lat' =>$this->db->escape_str($this->input->post('s')),
						  'flag' =>1
					);
			$q = $this->model_app->insert('tbl_rumahsakit_detil',$data);
			if ($q) {
				redirect('administrator/rs/berhasil','refresh');
			}else{
				redirect('administrator/rs/gagal','refresh');
			}
		}else{
			cek_session_admin();
			$data['title'] = 'Tambah Data GIS Rumah Sakit';
			$data['prov'] = $this->model_app->provinsi()->result_array();
			$this->template->load('administrator/template','administrator/mod_rs/tambah_rs', $data);
		}
	}
	function ubah_rs()
	{
		...
		if (isset($_POST['submit'])) {
			$data = array(
						  'kd_prov'=>$this->input->post('provinsi'),
						  'kd_kab'=>$this->input->post('kabupaten'),
			...
			$where = array('autono'=>$this->input->post('id'));
			$q = $this->model_app->update('tbl_rumahsakit_detil',$data,$where);
			if ($q) {
				redirect('administrator/rs/berhasil','refresh');
			}else{
				redirect('administrator/rs/gagal','refresh');
			...
			cek_session_admin();
			$data['title'] = 'Ubah Data GIS Rumah Sakit';
			$data['row']   = $this->model_app->edit('tbl_rumahsakit_detil',array('autono'=>$id))->row_array();
			$data['prov'] = $this->model_app->provinsi()->result_array();
			$this->template->load('administrator/template','administrator/mod_rs/ubah_rs', $data);
		}
	}
	function hapus_rs()
	{
		cek_session_admin();
		$id = array('autono'=>$this->uri->segment(3));
		$q = $this->model_app->delete('tbl_rumahsakit_detil',$id);
		if ($q) {
				redirect('administrator/rs/berhasil','refresh');
		}else{
				redirect('administrator/rs/gagal','refresh');
