...
class Model_utama extends CI_model{
    ...
    public function view_where_group($table,$field,$data,$group,$order,$ordering)
    {
    ...
    function cari_berita($kata){
        ...
        $cari = "SELECT * FROM tbl_informasi a
                       WHERE ";
    ...
    function cari_file($kata){
        $pisah_kata = explode(" ",$kata);
        $jml_katakan = (integer)count($pisah_kata);
        $jml_kata = $jml_katakan-1;
        $cari = "SELECT * FROM tbl_files a WHERE ";
            for ($i=0; $i<=$jml_kata; $i++){
              $cari .= " a.judul LIKE '%".$pisah_kata[$i]."%'";
                if ($i < $jml_kata ){
                    $cari .= " OR "; 
