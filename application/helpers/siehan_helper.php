<?php 
    ...
    function deskrip($str)
    {
        $ci = & get_instance();
        $ci->load->library('encryption');
        return str_replace(array('-','_','~'), array('=','+','/'), $ci->encryption->dencrypt($str));
    }
...
function cek_terakhir($datetime, $full = false) {
    ...
    $param['log_ip']= $ci->input->ip_address(); //target
    $param['log_datetime']= date('Y-m-d H:i:s'); //target

    $ci->load->model('model_app');
    //save to database
    $ci->model_app->save_log($param);
    return $param;
