...
$idxbgt = mysqli_query($con, "SELECT benua, SUM(strength) AS total FROM tbl_countrystrength WHERE subcategory = 'Defense Budget' GROUP BY benua ORDER BY benua DESC") or die (mysqli_error());
...
while($d = mysqli_fetch_assoc($drillngt)){		
		...
		$sub = mysqli_query($con, "SELECT negara, SUM(strength) AS totals FROM tbl_countrystrength WHERE benua = '$d[benua]' AND subcategory='Defense Budget' AND strength != 0 GROUP BY negara ORDER BY strength DESC") or die (mysqli_error());
