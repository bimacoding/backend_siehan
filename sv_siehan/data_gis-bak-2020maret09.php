...
$data0 = mysqli_query($con, "SELECT * FROM master_satker_xy WHERE tipe_org IN('UO','MABES') AND  parent_gid = 0 ORDER BY kd_uo ");
while ($value0 = mysqli_fetch_array($data0)) {
	...
	$data1 = mysqli_query($con, "SELECT * FROM master_satker_xy WHERE kd_uo = '$value0[kd_uo]' AND parent_gid = 0 ORDER BY kd_uo");
	while ($value1 = mysqli_fetch_array($data1)) {
		...
		$data2 = mysqli_query($con, "SELECT * FROM master_satker_xy WHERE parent_gid = '".$value1['gid']."'");
		while ($value2 = mysqli_fetch_array($data2)) {
			...
			$data3 = mysqli_query($con, "SELECT * FROM master_satker_xy WHERE parent_gid = '".$value2['gid']."'");
			while ($value3 = mysqli_fetch_array($data3)) {
				...
				$data4 = mysqli_query($con, "SELECT * FROM master_satker_xy WHERE parent_gid = '".$value3['gid']."'");
